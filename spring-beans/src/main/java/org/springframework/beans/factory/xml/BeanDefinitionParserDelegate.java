...
public class BeanDefinitionParserDelegate {
	 ...
	 * 解析{@code <bean>}元素，解析过程有错则返回null，错误信息放在ProblemReporter中
	 */
	@Nullable
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			...
			// 解析元数据 <meta /> 当需要使用里面的信息时可以通过 key 获取
			parseMetaElements(ele, bd);
			// 解析 lookup-method 属性 <lookup-method /> 获取器注入，是把一个方法声明为返回某种类型的 bean 但实际要返回的 bean 是在配置文件里面配置的。该方法可以用于设计一些可插拔的功能上，解除程序依赖
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());
			// 解析 replaced-method 属性 <replaced-method /> 可以在运行时调用新的方法替换现有的方法，还能动态的更新原有方法的逻辑
			parseReplacedMethodSubElements(ele, bd.getMethodOverrides());

			// 解析构造函数参数 <constructor-arg /> 构造函数，配置文件中使用 constructor-arg 元素对其配置，该元素可以实现自动寻找对应的构造函数，并在初始化的时候将值当做参数进行设置
